...
class KikoManager(object):
    ...
    def __init__(self, facade=None):
        ...
        self._facade = facade
        self._factory = OperatorsFactory()
        self.register()
    ...
    def register(self):
        self._factory.register(staticoperator.StaticOperator)
        self._factory.register(curveoperator.CurveOperator)
        self._factory.register(bakeoperator.BakeOperator)
        self._factory.register(worldspaceoperator.WorldSpaceOperator)
    ...
    def export_to_file(self, file_name, objects=None, operators=None,
        ...
        ops = []
        operators = operators or self._factory.get_all_operator_names(
                                                    self._facade.get_app_name())
        for o in operators:
            op_c = None
            if isinstance(o, tuple):
                if OperatorsFactory.has_operator(o[0], version=o[1]):
                    op_c = self._factory.get_operator(o[0], o[1])
            else:
                op_ver = self._factory.get_latest_version(o)
                if not op_ver is None:
                    op_c = self._factory.get_operator(o, op_ver)
