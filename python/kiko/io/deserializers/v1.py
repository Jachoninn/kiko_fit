...
def register():
    _factory.register(staticoperator.StaticOperator)
    _factory.register(curveoperator.CurveOperator)
    _factory.register(bakeoperator.BakeOperator)
    _factory.register(worldspaceoperator.WorldSpaceOperator)
    

_factory = OperatorsFactory()
register()
...
class _DeserializerV1Helper:
    ...
    def _load_chunk(facade, chunk_, op_priority, o, start_frame, end_frame,
                    ...
                    channel=None):
        op_c = _factory.get_operator(chunk_.operator_name,
...
class DeserializerV1(BaseDeserializer):
    ...
    def get_root_from_data(self, data, flatten_hierarchy=False,
        ...
        for op in data[SERIALIZATION.KIKO_OPERATORS]:
            if not _factory.has_operator(op[0], op[1]):
